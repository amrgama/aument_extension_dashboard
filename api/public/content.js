//  all comments
// commentData.comment1=`تم فحص العينة بعد الاطلاع على كشف الحساب حيث تتمثل الحركة في  ${commentData.desc+"\n"+"برقم مرجع"+" "+commentData.refNumber+"\n"+" بتاريخ"+" "+commentData.date+" "+"تم مقارنة المستندات المؤيدة مع ماهو وارد بكشف الحساب والتأكد من صحتها ومطابقتها "}`;
// commentData.comment2=` ${commentData.desc+"\n"+"برقم مرجع"+" "+commentData.refNumber+"\n"+" بتاريخ"+" "+commentData.date+" "}`;
// commentData.comment3= ` ${commentData.desc+"\n"+"برقم مرجع"+" "+commentData.refNumber+"\n"+" بتاريخ"+" "+commentData.date+" "}`;
// commentData.comment4= `تم مطابقة  ${commentData.desc+"\n"+" بتاريخ"+" "+commentData.date+"\n"+"بقيمة "+" "+commentData.value+" "+"، "+"برقم مرجع"+" "+commentData.refNumber+" "}`;
// commentData.comment5=ifPOrNValue;
// commentData.comment6=`تم الإطلاع علي المستند   ${commentData.desc+"\n"+" بتاريخ"+" "+commentData.date+"\n"+"بقيمة "+" "+commentData.value+" "+"، "+"برقم مرجع"+" "+commentData.refNumber+" "}`;

// textarea.onkeydown=(e)=> {
//   e.stopPropagation();
//   // for mark if entered is right value or not
//   let flag=true;
//   // for remove wrong value word from textarea
//   if(textarea.value.includes("wrong value")) {
//     textarea.value="";
//     textarea.style.backgroundColor="white";
//   }
//   // for handle entered value after user press enter keyword and transform it to array 
...
//         if(text[parseInt(item.textContent)] != undefined){
//           commentData.outerDescriptionColumn.push(text[parseInt(item.textContent)]);    
//         }
//       }); 
//       flag=true;
//       // for know if the entered value is right value and base on that make a permission for comment
//       commentData.flag=true;
//       console.log("ks  "+flag);
//     }else{
//       flag=false;
//       commentData.flag=false;
//       // for send a message to user if he entered empty value
//       textarea.value="wrong value";
//       textarea.style.backgroundColor="red";
//     }
//     // for close textarea box when user press enter
//     if(flag){
//       // closeButton.classList.remove("close-button"); 
//       outerTextarea.classList.remove("textarea-popup");
//       // for make a select on group-A when user press enter
//       groupsUl.firstElementChild.classList.toggle("select-color",true);
//       stop=false;
//     }
//   }
// };

// if(commentData.value.includes("(")){
    //   const start = commentData.value.indexOf("(") + 1;
    //   const end = commentData.value.indexOf(")");
    //   commentData.value.value= commentData.value.value.slice(start, end);

    //   ifPOrNValue=`تم مطابقة  ${commentData.desc+"\n"+" بتاريخ"+" "+commentData.date+"\n"+"بقيمة "+" "+commentData.value+" "+"برقم مرجع"+" "+commentData.refNumber+" "+ `   تم اثبات القيد من حساب      الى حساب ${commentData.accountName}`}
    //   `;
    // } 
    // else{
    //   ifPOrNValue=`تم مطابقة  ${commentData.desc+"\n"+" بتاريخ"+" "+commentData.date+"\n"+"بقيمة "+" "+commentData.value+" "+"برقم مرجع"+" "+commentData.refNumber+" "+`تم القيد من حساب ${commentData.accountName} الى حساب `}
    //   `;
    // }
